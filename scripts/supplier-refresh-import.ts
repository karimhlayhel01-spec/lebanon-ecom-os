/**
 * One-shot founder recovery: refresh Import leads for a single owned SKU.
 * Usage:
 *   npx tsx scripts/supplier-refresh-import.ts --skuId=<id> [--confirm]
 *   npx tsx scripts/supplier-refresh-import.ts --active
 *
 * Requires SUPPLIER_LIVE_LEADS=1 and SERPER_API_KEY. Does not wipe all workspaces.
 */

import { db, ensureMigrated, schema } from "@/db";
import { eq } from "drizzle-orm";
import { refreshImportLeads } from "@/lib/supplier/service";
...
async function main() {
  ...
  const res = await refreshImportLeads(workspaceId, skuId!, {
